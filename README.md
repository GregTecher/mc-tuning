...
没接触过图形学知识，有些mod没有去翻源码，纯粹从逻辑上猜测，若有不当之处，欢迎大佬指正补充。
...
吹爆，可劲堆机器吧，完全不掉FPS，不掉TPS。机器都不是可渲染实体，我就问还有谁？IC貌似也是。GregBlock1.4.6用的GTCE有些机器会严重影响TPS，之后版本已经修复。个人做了个1.4.6修复版，为避免引起纠纷，不在此处发布。
...
掉TPS，不用的时候请按左上角暂停。不然会吃TPS。还有IF有时候会莫名奇妙吃FPS，视角一移向IF机器，FPS就疯狂的掉。
...
魔改了能量传输机制，理论上所有供能设备（如能量输出仓，电池盒，变压器）的数量在原有的基础上可以将数量提升16倍而不会降低tps。作者拒绝了这个想法。
同时魔改了配方查询机制，使得非运行态且输入槽有内容的机器对CPU的负载大大降低，以组装机为最，从1kus降到10us一下，效果显著。
作者之后版本已经修复，然而GregBlock留在了124版本。

话说在这个从源码编译不违反LGPL3协议吧，若有违反还请提醒，立马删除。
